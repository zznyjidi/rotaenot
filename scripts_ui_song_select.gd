...
func _update_info_panel(song: Dictionary):
	...
	_load_and_scale_preview_image(song)
...
func _start_game():
	...
	get_tree().change_scene_to_file("res://scenes_ui_loading_transition.tscn")
...
func _go_back():
	...
	get_tree().change_scene_to_file("res://scenes_main_menu_diamond_menu.tscn")
...
func _load_and_scale_preview_image(song_data):
	...
	# Use preloader for HTML5 compatibility
	var texture
	if song_data is Dictionary:
		texture = ImagePreloader.get_cover_for_song(song_data)
	else:
		texture = ImagePreloader.get_cover_from_path(str(song_data))
	if not texture:
		print("Failed to load image for song")
...
func _start_preview_playback():
	...
	if current_index >= 0 and current_index < songs.size():
		...
		if song.has("music_path") and song.music_path != "":
			...
			# Load and play the music using preloader
			var stream = MusicPreloader.get_music(music_path)
			if stream:
